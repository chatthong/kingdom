...
commands/                  # 10 slash commands (5 core + 5 role-bootstrap)
...
.kingdom/.setting/         # canonical source — copied into workspace by /kingdom:init
  ...
  rules/                   # one rule per file (R01..R52) + index.md registry (Tier 1/2/3; Tier-1 capped at 10) — v0.34.0
  ...
  reference/               # cmux.md git.md skill-routing.md role-bootstrap.md (shared guides) — v0.35.0, +role-bootstrap v0.39.0
...
0. **`_bounded_wait` sequential-loop rc on timeout (latent, v0.37.0 audit).** The helper polls PIDs one at a time; on a wall-clock timeout it returns 124 correctly, but the non-zero exit codes of *already-finished* later PIDs are never collected (only the current-PID's error can surface). Not a crash — wrong-ish rc under a specific load pattern. Fixing it means restructuring to poll all PIDs concurrently; deferred because it changes concurrency semantics and no caller currently depends on the per-PID rc. (The earlier "kill -9 overwrites rc with 137" claim was refuted — the early `return 124` prevents it.)
...
- **The 52 rules:** `.kingdom/.setting/rules/` (one per file + `index.md` registry). Tier 1 = iron-clad (10, capped — see legend); Tier 2 = strong defaults (37); Tier 3 = conventions (5).
