...
- **Talking to the King (R55):** when a story needs a cross-story decision only the King can make (scope conflict with another story, a dependency the King must sequence, an unresolvable R49 conflict), post it to the shared broker inbox instead of stalling: `inbox_send king question "story-<id>" yes "..."` (or `flag`). The Senior can also bell any pod worker the same way: `inbox_send worker-N info "story-<id>" no "..."`. Keep working on continuable parts, and check `inbox_list --to senior-N` at story start, when blocked, and before the closer. This is in ADDITION to the `⛔`-state blocked escalation (Step 5 below) — the inbox is for *questions*, `⛔` + a withheld sentinel is for a fully-blocked story.
