...
**R52 carve-out:** the spawn-time `/kingdom:self-co-worker` self-ground (R52) is a read-only GROUNDING message, not a task dispatch, so it does NOT break dormancy — the co-worker still waits for `pair on co-worker-N` before doing any work.
