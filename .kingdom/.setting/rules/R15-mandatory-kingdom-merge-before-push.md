...
> [!NOTE]
> The overlay is the King's **self-initiated default**. A direct human instruction to "merge" / "merge X to kingdom" overrides it — the King obeys verbatim and makes real merge commits. See the carve-out in [R04](R04-never-edit-or-commit.md) for the full protocol. The overlay-as-review-surface rule (above) still applies once the King returns to self-directed behaviour after the explicit instruction is fulfilled.
